...
w_speed_bigger = [1800;1500;300;300;200;200];
w_speed_smaller = [w_speed_bigger(6);w_speed_bigger(6);...
...
for iter = 1:N_iteration
    ...
    if iter == 1
        [GK_bigger, GM_bigger, GC_bigger] = FEM_matrices_bigger( );                  % create GK, GC, and GM for the bigger domain
    ...
    for iter_force = 1:N_force
        ...
        A_min = 50; A_max = 150;
        % A = round(A_min + (A_max-A_min)*rand(1,1));
        A = randi([A_min,A_max],1,1);                                                   % Random - Ricker's amplitude

        freq_min = 1; freq_max = 5;
        % freq = randi([freq_min,freq_max],1,1);                                         % Random - frequency
    ...
    %  [u_curt_history_big] = solve_u_DRM2(GF_m);
    %  %   >>>>>>>>>>>>>>>>>>>>>>>>>>>>> 2nd PART <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
