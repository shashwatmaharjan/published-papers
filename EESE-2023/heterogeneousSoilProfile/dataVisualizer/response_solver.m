...
filename = strcat('hetero_', category, '_response_3.pdf');

%Which set of data? %the number 1 means the first set of sample data
% sample = 1;         %%% CHANGE THIS LINE (NUMBER)

target1 = target(sample,:,:);
% target1 = reshape(target1, [], size(target1,2))';       % rearrange the slice into a 2D matrix
target1 = reshape(target1, 126, 501);

prediction1 = prediction(sample,:,:);
% prediction1 = reshape(prediction1, [], size(prediction1,2))'; % rearrange the slice into a 2D matrix
