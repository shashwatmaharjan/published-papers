...
%%
N_void = 1;                                      % Creates number of voids of random number.
if N_void==0
    ...
    for p= 1: N_void
        ...
        a= 0.05; % +(0.08-0.03).*rand(1,1);                                             % Major axis= m/2
        b= 0.015; % +(0.015-0.01).*rand(1,1);                                            % Minor axis= n/2
        H_ellipse=  0.5; % a+ ((L- a)-a).*rand(1,1) ;                                 % Random center x ellipse
        K_ellipse= 0.15 ; % Y_loc(loc);%0.15;  %H/2 + ((H- a)-H/2).*rand(1,1) ;                                  % Random center y ellipse
        Angle= 5; % -5 + (5-(-5)).*rand(1,1);                                       % Random angle ellipse
