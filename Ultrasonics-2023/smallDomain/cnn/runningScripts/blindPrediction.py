...
def plot_prediction(sample_number, predicted_data, file_directory):
    ...
    plot = plt.figure()
    ax = plt.axes()
    im = plt.imshow(predicted_data[sample_number-1, :, :], cmap='binary', origin='lower', vmin=0, vmax=1, extent=[0, 0.1, 0, 0.05])
...
def main():
    ...
    organized_data_file_directory = main_file_directory + '/dataset/voidSpecificDataset'
    ...
    # displacement_data = get_data(blind_test_data_file_directory)
